...
// ═══════════════════════════════════════════════════════════
// strQ.app — Privacy policy
// Server-rendered: leest content/privacy/{locale}.md en
// rendert als donker-thema prose. Geen JS nodig op de client.
// ═══════════════════════════════════════════════════════════
...
async function loadPolicy(locale: string): Promise<string | null> {
...
export default async function PrivacyPage({
  ...
  const markdown = await loadPolicy(locale);
  if (!markdown) notFound();

  const t = await getTranslations({ locale, namespace: "privacy_page" });

  // Marked: GFM is default in v18 (tables, strikethrough). breaks: false (paragraph based).
